...
function StoreContacts() {
   ...
   const handleSelectGroup = (dropdownValue: GroupDrop) => {
      const alreadySelected = groupName.includes(dropdownValue.group_name);
      if (alreadySelected) {
         setGroupName(groupName.filter(group => group !== dropdownValue.group_name));
         setGroupId(groupId.filter(id => id !== dropdownValue.id));
         console.log(groupId, "GropId")
      } else {
         setGroupName([...groupName, dropdownValue.group_name]);
   ...
   const superAdminContactCreate = () => {
      ...
      let apiData = {
         ...
         groupdetails: groupName.map((group) => {
            const groupData = groupDropDown.find(item => item.group_name === group);
            return {
               groupName: group,
               groupId: groupData ? parseInt(groupData.id) : null
            };
         })
      };
      ...
      VendorAPI.contactGroupDropdownAPI()
         .then((responseData: any) => {
            if (responseData.apiStatus.code === '200') {
               ...
               console.log(responseData?.result?.GroupDataDropDown)
            } else {
               toast.error(responseData.apiStatus.message);
            }
         })
   ...
   const handleRemoveGroup = (groupNameToRemove: string, groupIdToRemove: any) => {
      setGroupName(groupName.filter(group => group !== groupNameToRemove));
      setGroupId(groupId.filter(id => id !== groupIdToRemove));
   };
                  ...
                  <div className="row modal-container-size modal-body vendorcontact-modal-body">
                     <div className="row mt-n4">
                        ...
                        <div className="col-md-6 login-input-group">
                           <div className="vendor-create-container dropdown">
                              <input type="text" id="vendor-crt-input"
                                 style={submit && countryId.length == 0 ? { borderColor: "red" } : { borderColor: "" }}
                                 onChange={(e) => setCountryId(e.target.value)} value={countryId} className="vendor-crt-input" placeholder=" " required />
                              <label htmlFor="vendor-crt-input" className="vendor-crt-label"><i className="fa-solid fa-id-card-clip"></i> Country</label>
                              {submit && countryId.length == 0 ? (
                                 <div className="text-danger error-message-required">Country is required </div>
                              ) : (
                                 <></>
                              )}
                        ...
                        <div className="col-md-6 login-input-group">
                           <div className="vendor-create-container">
                              <input type="text" id="vendor-crt-input"
                                 style={submit && languageCode.length == 0 ? { borderColor: "red" } : { borderColor: "" }}
                                 onChange={(e) => setLanguageCode(e.target.value)} className="vendor-crt-input" value={languageCode} placeholder=" " required />
                              <label htmlFor="vendor-crt-input" className="vendor-crt-label"><i className="fa-solid fa-at"></i> Language Code</label>
                           </div>
                           {submit && languageCode.length == 0 ? (
                              <div className="text-danger error-message-required">LanguageCode is required </div>
                           ) : (
                              <></>
                           )}
                        </div>
                        ...
                        <div className="col-md-6 login-input-group">
                           <div className="vendor-create-container dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                              <input type="text" id="vendor-crt-input" readOnly
                                 style={submit && storeId.length == 0 ? { borderColor: "red" } : { borderColor: "" }}
                                 onClick={handleGetStoreDrop} value={storeName} className="vendor-crt-input" placeholder=" " required />
                           ...
                           {submit && storeId.length == 0 ? (
                              <div className="text-danger error-message-required">StoreName is required </div>
                           ) : (
                              <></>
                           )}
                        </div>
                        <div className="col-md-6 login-input-group">
                           <div className="vendor-create-container dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                              <input type="text" id="vendor-crt-input" readOnly
                                 ...
                                 onClick={handleGetGroupDrop} value={groupName.join(', ')} className="vendor-crt-input" placeholder=" " required />
